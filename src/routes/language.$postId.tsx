...
const postFiles = import.meta.glob('../posts/language/*/index.md', { 
  query: '?raw',
  import: 'default',
  eager: false
});
...
const imageFiles = import.meta.glob('../posts/language/**/*.{png,jpg,jpeg,gif,svg,webp}', {
  eager: true,
  import: 'default'
});
...
export default function LanguagePost() {
  ...
  useEffect(() => {
    const loadPost = async () => {
      ...
      const postPath = `../posts/language/${postId}/index.md`;
  ...
  return (
    <article>
      <Link to="/language">← Back to Language</Link>
        ...
        <ReactMarkdown
          components={{
            img: ({node, ...props}) => {
              ...
              let src = props.src;
              if (src?.startsWith('./')) {
                const imagePath = `../posts/language/${postId}/${src.slice(2)}`;
